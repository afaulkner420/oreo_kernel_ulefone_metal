...
#if defined(CONFIG_GTP_SLIDE_WAKEUP)
static int tpd_gesture_support_keys[] =  {KEY_F1/*KEY_LEFT*/, KEY_F3/*KEY_RIGHT*/, KEY_F2/*KEY_UP*/, KEY_F4/*KEY_DOWN*/, KEY_U, KEY_O, KEY_W, KEY_M, KEY_E, KEY_C, KEY_S, KEY_V, KEY_Z, KEY_POWER};
#endif
...
#if 0
	of_get_gt9xx_platform_data(&client->dev);//use device
...
#if defined(CONFIG_GTP_SLIDE_WAKEUP)
{
	int key_index, key_count;

	key_count = sizeof(tpd_gesture_support_keys) / sizeof(tpd_gesture_support_keys[0]);
	
	for (key_index = 0; key_index < key_count; key_index++)
	{
	        input_set_capability(tpd->dev, EV_KEY, tpd_gesture_support_keys[key_index]);
	}
}
#endif
	...
	//zhuyifeng, report fw version
	{
		#define GUP_REG_PID_VID						 0x8140
		u8 buf[8] = {0};
		char info[32] = {0};
		extern void TP_SetTP_Name_info(char *tp_name);
		
		ret = gtp_i2c_read_dbl_check(client, GUP_REG_PID_VID, buf, 6);
		
		if (0/*FAIL*/ == ret)
		{
			sprintf(info, "gt9xx V0");
		}
		else
		{
			sprintf(info, "gt9xx V%d", (buf[4] + (buf[5] << 8)));
		}
		
		TP_SetTP_Name_info(info);
	}
...
static void check_gesture(int gesture_id)
{
	...
	{
		case 0xba:
			keycode = KEY_F2;//KEY_UP;
			break;
		case 0xaa:
			keycode = KEY_F3;//KEY_RIGHT;
			break;
		case 0xab:
			keycode = KEY_F4;//KEY_DOWN;
			break;
		case 0xbb:
			keycode = KEY_F1;//KEY_LEFT;
			break;
		case 'o':
			keycode = KEY_O;
			break;
...
static int touch_event_handler(void *unused)
{
	...
	do {
		...
		#if 0//defined(CONFIG_GTP_SLIDE_WAKEUP)  //pay attention
            if (DOZE_ENABLED == doze_status)
            {
                break;
            }
...
//							msleep(5);
							...
							(doze_buf[2] == 0xAB) || (doze_buf[2] == 0xBA) )
						{
							...
							doze_status = DOZE_WAKEUP;
							sprintf(tpgesture_value,direction[type]);
		...
		if (tpd_dts_data.use_tpd_button) 
		{
	        ...
	        if (key_value || pre_key)
	        {
	            for (i = 0; i < TPD_KEY_COUNT; i++)
	            {
	                ...
	                {
	                    tpd_down(tpd_dts_data.tpd_key_dim_local[i].key_x, tpd_dts_data.tpd_key_dim_local[i].key_y, 0,0);
	                    printk("[wsl]tpd_dts_data.tpd_key_dim_local[%d].key_x=%d,tpd_dts_data.tpd_key_dim_local[%d].key_y=%d,tpd_dts_data.tpd_key_local[%d]=%d \n",i,tpd_dts_data.tpd_key_dim_local[i].key_x,i,tpd_dts_data.tpd_key_dim_local[i].key_y,i,tpd_dts_data.tpd_key_local[i]);
			}
	            }

	            if ((pre_key != 0) && (key_value == 0))
	            {
	                   tpd_up(tpd_dts_data.tpd_key_dim_local[0].key_x, tpd_dts_data.tpd_key_dim_local[0].key_y, 0);
	            }
...
exit_work_func:
		...
		if(mutex_is_locked(&i2c_access))       //by liyu
		       mutex_unlock(&i2c_access);
		enable_irq(touch_irq);
exit_unlock:
		//gtp_irq_enable();
...
/* wakeup touch ic by int pin or hw reset */
	while (retry++ < 3) {
#if 0//def CONFIG_GTP_GESTURE_WAKEUP
		if (gesture_data.enabled) {
...
#endif
	GTP_INFO("GTP zy System suspend.");

	tpd_halt = 1;
	mutex_lock(&i2c_access);
	//gtp_irq_disable();
	disable_irq(touch_irq); //zy
...
static int __init tpd_driver_init(void)
{
	...
	tpd_get_dts_info();
	memcpy(tpd_dts_data.tpd_key_local, tpd_keys_local, sizeof(tpd_keys_local));
	memcpy(tpd_dts_data.tpd_key_dim_local, tpd_keys_dim_local, sizeof(tpd_keys_dim_local));
