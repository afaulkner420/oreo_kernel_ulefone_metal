...
extern int is_ili7802_hd720_dsi_vdo_boe; //zhangchongyong add 20160412
extern int is_hx8394f_dsi_vdo_auo;//xiehaifei add 20160627 for hx8394f esd
...
#if 1//defined(HX8394F_HD720_DSI_VDO_AUO)
int ddp_dsi_build_cmdq(DISP_MODULE_ENUM module, void *cmdq_trigger_handle, CMDQ_STATE state)
{
	...
	int j 	= 0;//xiehaifei
	 DSI_T0_INS t1;//xiehaifei
	uint32_t recv_data_cnt;
	uint32_t data_arr[4] = {0,0,0,0};
	static uint32_t data_paras[2] = {0x0f,0xf0};
 	unsigned char packet_type;
	//end by xiehaifei
	...
	if (state == CMDQ_BEFORE_STREAM_SOF) {
		...
		//zhangchongyong add Z1 esd deal add start
		if (is_ili7802_hd720_dsi_vdo_boe == 1)
		{
			/* write DSI CMDQ */
			...
			0x00000000);
			DSI_OUTREG32(cmdq_trigger_handle, &DSI_CMDQ_REG[dsi_i]->data[2],
			0x00000000);		
			DSI_OUTREG32(cmdq_trigger_handle, &DSI_CMDQ_REG[dsi_i]->data[3],
			0x00000000);
			DSI_OUTREG32(cmdq_trigger_handle, &DSI_CMDQ_REG[dsi_i]->data[4],
			0x00000000);
			DSI_OUTREG32(cmdq_trigger_handle, &DSI_CMDQ_REG[dsi_i]->data[5],
			0x00000000);		
			DSI_OUTREG32(cmdq_trigger_handle, &DSI_CMDQ_REG[dsi_i]->data[6],
			0x00000000);	
			DSI_OUTREG32(cmdq_trigger_handle, &DSI_CMDQ_REG[dsi_i]->data[7],
			0x00000000);		
			DSI_OUTREG32(cmdq_trigger_handle, &DSI_CMDQ_REG[dsi_i]->data[8],
			0x00000000);					     
			DSI_OUTREG32(cmdq_trigger_handle, &DSI_CMDQ_REG[dsi_i]->data[9],
			0x00000008);	
			DSI_OUTREG32(cmdq_trigger_handle, &DSI_CMDQ_REG[dsi_i]->data[10],
			0x00000008);
			DSI_OUTREG32(cmdq_trigger_handle, &DSI_CMDQ_REG[dsi_i]->data[11],
			0x00000008);					     					     				     		     		     	     
			DSI_OUTREG32(cmdq_trigger_handle, &DSI_CMDQ_REG[dsi_i]->data[12],
			0x00013700);
			DSI_OUTREG32(cmdq_trigger_handle, &DSI_CMDQ_REG[dsi_i]->data[13],
			AS_UINT32(&t0));
			DSI_OUTREG32(cmdq_trigger_handle, &DSI_REG[dsi_i]->DSI_CMDQ_SIZE, 14);
		}
		else
		{
			/* write DSI CMDQ */
		...
		//zhangchongyong add Z1 esd deal add end
			...
			*/
			...
			printk("<XHF>packet_type=%x\n",packet_type);
   			if(packet_type == 0x1A || packet_type == 0x1C)
   			{
				...
				j = 0;
				printk("<DSI>recv_data_cnt=%x\n",recv_data_cnt);
				while(recv_data_cnt--)
				{

					if (is_hx8394f_dsi_vdo_auo == 1)
					{
						printk("<DSI>is_hx8394f_dsi_vdo_auo\n");
						if(i!=2)
						{
							    if(data_arr[j] == dsi_params->lcm_esd_check_table[i].para_list[j]){
								  printk("<hx8394f> esd pass---1 !\n");	
			    					   ret = 0; // esd pass
			   			 	    }
			   				    else{
								   printk("<hx8394f> esd fail---1 !\n");	
			    					   ret = 1; // esd fail
			    					   break;
			   				    }					
						}
						else
						{
							   printk("data_arr[%d] =%x\n",j,data_arr[j] );
							    if((data_arr[j] == dsi_params->lcm_esd_check_table[i].para_list[j]) || (data_arr[j] == data_paras[j])){
								  printk("<hx8394f> esd pass---2 !\n");		
							    	   ret = 0; // esd pass
							    }
							    else{
								   printk("<hx8394f> esd fail---2!\n");	
					...
					{
						if(data_arr[j] == dsi_params->lcm_esd_check_table[i].para_list[j]){
							printk("<DSI>muti  cmd esd pass---1 !\n");	
							ret = 0; // esd pass
						}
						else{
							printk("<DSI> muti  cmd esd fail---1 !\n");	
							ret = 1; // esd fail
							break;
						}				
				...
				if(recv_data_cnt > (dsi_params->lcm_esd_check_table[i].count))
				{
					...
					// clear rx data
     					// DSI_OUTREG32(NULL, &DSI_REG[dsi_i]->DSI_RX_DATA0,0);
     					  printk("<xhf>signle cmd  esd pass---!\n");	
					ret = 0; // esd pass
				}
				else
				{
					 printk("<xhf>signle cmd  esd fail---!\n");	
     					ret = 1; // esd fail
     					break;
				}
         		}
